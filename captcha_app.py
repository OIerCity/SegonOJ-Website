...
class Captcha():
    ...
    def make_captcha(self,uid):
        ...
        # 获取字体对象
        font = ImageFont.truetype('./static/general/font/Lato-Regular-15.ttf',self.font_size)
        # 获取画笔对象
...
def captcha():
    ...
    if len(find_captcha({'uid':user['uid']})) == 0:
        ...
        return send_from_directory(os.path.join('./static/captcha', 'captcha'),str(user['uid'])+'.png', as_attachment=True)     
    else:
        c_captcha.update_one({'uid':user['uid']},{'$set':{'captcha':captcha_text}})
        return send_from_directory('./static/captcha',str(user['uid'])+'.png', as_attachment=True)     
